import { useState } from "react";
import data from "../assets/data";
import { useClick } from "./useClick";
function OrderModal({ modalMenu, setModalOn }) {
  const { handleCart, handleOptions, handleQuantity, options, quantity } =
    useClick();
  const itemOptions = data.options;
  console.log(options);
  return (
    <>
      {modalMenu ? (
        <section className="modal-backdrop" onClick={() => setModalOn(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-item">
              <img src={modalMenu.img} />
              <div>
                <h3>{modalMenu.name}</h3>
                <div>{modalMenu.description}</div>
              </div>
            </div>
            <ul className="options">
              {Object.keys(itemOptions).map((el) => (
                <Option
                  key={el}
                  options={options}
                  handleOptions={handleOptions}
                  name={el}
                  itemOptions={itemOptions[el]}
                />
              ))}
            </ul>
            <div className="submit">
              <div>
                <label htmlFor="count">개수</label>
                <input
                  id="count"
                  type="number"
                  value={quantity}
                  min="1"
                  onChange={handleQuantity}
                />
              </div>
              <button onClick={() => handleCart(modalMenu)}>
                {/*html이지만 자바스크립트로 함수식을 써야하므로 익명함수 사용 */}
                장바구니 넣기
              </button>
            </div>
          </div>
        </section>
      ) : null}
    </>
  );
}
function Option({ name, options, handleOptions, itemOptions }) {
  return (
    <li className="option">
      {name}
      <ul>
        {itemOptions.map((option, idx) => (
          <li key={option}>
            <input
              type="radio"
              name={name}
              checked={options[name] === idx}
              onChange={() => handleOptions(name, idx)}
            />
            {option}
          </li>
        ))}
      </ul>
    </li>
  );
}
export default OrderModal;

//handleCart(modalMenu);
